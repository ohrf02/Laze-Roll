...
static constexpr float MIN_RANGE = 0.95;
static constexpr float MAX_RANGE = 1.05;

void setup()
{
    Serial.begin(115200);
    pinMode(STATUS_LED, OUTPUT); // Status LED
    pinMode(LASER_PIN, OUTPUT);  // Laser LED
...
void loop() //Main Loop
{
    if ((millis() - timer) >= 20) // Main loop runs at 50Hz
    {
        ...
        if (MAG_Heading >= MIN_RANGE && MAG_Heading <= MAX_RANGE)
        {
